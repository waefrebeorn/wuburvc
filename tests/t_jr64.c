/* t_jr64.c — A/B: conv jr-block 32 (current) vs 64 (fewer weight broadcasts
 * per sample, more ILP). Same accumulation order per element → byte-identical.
 * Build: gcc -O3 -mavx2 -mfma -march=znver2 -fopenmp t_jr64.c -o t_jr64.exe
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void conv_jr(const float *in, int in_ch, int n, const float *w,
                    int out_ch, int k, int pad, int dil, float *out, int JR) {
...
#pragma omp parallel for schedule(dynamic, 4) num_threads(8) if(n_out >= TILE)
    for (int jb = 0; jb < n_out; jb += TILE) {
        ...
        int jr_max = n - JR - (k - 1) * dil + pad;
        if (jr_max > j_in_hi - JR) jr_max = j_in_hi - JR;
        for (int oc0 = 0; oc0 < out_ch; oc0 += OC_BLK) {
            ...
            for (; jr <= jr_max; jr += JR) {
                __m256 a[OC_BLK][8];
                for (int oc = 0; oc < n_oc; oc++)
                    for (int v = 0; v < JR / 8; v++)
                        a[oc][v] = _mm256_loadu_ps(orow0[oc] + jr + v * 8);
                for (int ick = 0; ick < in_ch; ick++) {
                    ...
                    for (int tap = 0; tap < k; tap++) {
                        ...
                        __m256 iv[8];
                        for (int v = 0; v < JR / 8; v++)
                            iv[v] = _mm256_loadu_ps(ir2 + v * 8 + tap * dil);
                        for (int oc = 0; oc < n_oc; oc++) {
                            float wt = wv0[oc][(size_t)ick * k + tap];
                            __m256 wv8 = _mm256_set1_ps(wt);
                            for (int v = 0; v < JR / 8; v++)
                                a[oc][v] = _mm256_fmadd_ps(iv[v], wv8, a[oc][v]);
                        }
                    }
                }
                for (int oc = 0; oc < n_oc; oc++)
                    for (int v = 0; v < JR / 8; v++)
                        _mm256_storeu_ps(orow0[oc] + jr + v * 8, a[oc][v]);
            }
            for (; jr < j_in_hi; jr++)
                for (int oc = 0; oc < n_oc; oc++) {
                    const float *wv = wv0[oc];
...
int main(int argc, char **argv) {
    ...
    conv_jr(in, in_ch, n, w, out_ch, k, pad, dil, o1, 32);
    conv_jr(in, in_ch, n, w, out_ch, k, pad, dil, o2, 64);
    ...
    printf("jr64 vs jr32: maxdiff=%.9f nbad=%d\n", mx, nbad);
    double macs = (double)out_ch * in_ch * k * n_out * reps;
    double t1 = omp_get_wtime();
    for (int r = 0; r < reps; r++) conv_jr(in, in_ch, n, w, out_ch, k, pad, dil, o1, 32);
    double t2 = omp_get_wtime();
    for (int r = 0; r < reps; r++) conv_jr(in, in_ch, n, w, out_ch, k, pad, dil, o2, 64);
    double t3 = omp_get_wtime();
    printf("jr32: %.3fs  %.1f GMAC/s\n", t2 - t1, macs / 1e9 / (t2 - t1));
    printf("jr64: %.3fs  %.1f GMAC/s\n", t3 - t2, macs / 1e9 / (t3 - t2));
    printf("SPEEDUP: %.2fx\n", (t2 - t1) / (t3 - t2));
